...
class C2FNet(nn.Layer):
    ...
    def forward(self, x, heatmaps, label=None):
        ...
        if self.training:
            ...
            mask_regions = paddle.reshape(mask_regions, shape=[-1, self.kernel_sizes[0] * self.kernel_sizes[1]])

            img_regions = F.unfold(x, kernel_sizes=self.kernel_sizes, strides=self.training_stride, paddings=0,
                                   dilations=1, name=None)
            img_regions = paddle.transpose(img_regions, perm=[0, 2, 1])
            img_regions = paddle.reshape(img_regions, shape=[-1, 3 * self.kernel_sizes[0] * self.kernel_sizes[1]])

            label_regions = F.unfold(label, kernel_sizes=self.kernel_sizes, strides=self.training_stride, paddings=0,
                                     dilations=1, name=None)
            label_regions = paddle.transpose(label_regions, perm=[0, 2, 1])
            label_regions = paddle.reshape(label_regions, shape=[-1, self.kernel_sizes[0] * self.kernel_sizes[1]])

            # Silin：3、通过coase_seg_map找到包含小目标的pacth
            mask_regions_sum = paddle.sum(mask_regions, axis=1)
            mask_regions_selected = paddle.where(mask_regions_sum > 0, paddle.ones_like(mask_regions_sum),
                                                 paddle.zeros_like(mask_regions_sum))
            final_mask_regions_selected = paddle.zeros_like(mask_regions_selected).astype('bool')
            ...
            if paddle.sum(mask_regions_selected) >= theld:
                ...
                selected_img_regions = paddle.reshape(selected_img_regions,
                                                      shape=[theld, 3, self.kernel_sizes[0], self.kernel_sizes[1]])

                selected_label_regions = label_regions[final_mask_regions_selected]
                selected_label_regions = paddle.reshape(selected_label_regions, shape=[theld, self.kernel_sizes[0],
                                                                                       self.kernel_sizes[1]]).astype(
                    'int32')
                ...
                selected_label_regions = label_regions[final_mask_regions_selected]
                selected_label_regions = paddle.reshape(selected_label_regions, 
        ...
        # Silin: Inference Process
            ...
            mask_regions = F.unfold(heatmap, kernel_sizes=self.kernel_sizes, strides=self.kernel_sizes[0], paddings=0,
                                    dilations=1, name=None)
            mask_regions = paddle.transpose(mask_regions, perm=[0, 2, 1])
            mask_regions = paddle.reshape(mask_regions, shape=[-1, self.kernel_sizes[0] * self.kernel_sizes[1]])

            img_regions = F.unfold(x, kernel_sizes=self.kernel_sizes, strides=self.kernel_sizes[0], paddings=0,
                                   dilations=1, name=None)
            img_regions = paddle.transpose(img_regions, perm=[0, 2, 1])
            img_regions = paddle.reshape(img_regions, shape=[-1, 3 * self.kernel_sizes[0] * self.kernel_sizes[1]])
            ...
            # Silin: 3、如果有小目标，则需要对这些patch进行refine
                ...
                ori_fea_regions = F.unfold(ori_heatmap, kernel_sizes=self.kernel_sizes, strides=self.kernel_sizes[0],
                                           paddings=0,
                                           dilations=1, name=None)
                ori_fea_regions = paddle.transpose(ori_fea_regions, perm=[0, 2, 1])
                ori_fea_regions = paddle.reshape(ori_fea_regions,
                                                 shape=[-1, self.num_cls * self.kernel_sizes[0] * self.kernel_sizes[1]])
                
                # Silin: 5、选择包含小目标的img_patches
                selected_img_regions = img_regions[mask_regions_selected]
                selected_img_regions = paddle.reshape(selected_img_regions,
                                                      shape=[paddle.shape(selected_img_regions)[0], 3,
                ...
                selected_fea_regions = paddle.reshape(selected_fea_regions,
                                                      shape=[paddle.shape(selected_fea_regions)[0], self.num_cls,
                                                             self.kernel_sizes[0], self.kernel_sizes[1]])
                # Silin: 7、前向传播
                feat_list = self.backbone(selected_img_regions)
                bgfg = self.head_fgbg(feat_list)
                ...
                bg_binary, fg_binary = paddle.chunk(binary_fea, chunks=2, axis=1)
               
                # Silin: 9、切分coase_map概率
                front, ship, mid, lv, sv, hl, swp, mid2, pl, hb = paddle.split(selected_fea_regions,
                                                                num_or_sections=[1, 1, 6, 1, 1, 1, 1, 2, 1, 1], axis=1)
                ...
                selected_fea_regions = paddle.concat(x=[front, ship, mid, lv, sv, hl, swp, mid2, pl, hb], axis=1)
                selected_fea_regions = paddle.reshape(selected_fea_regions,
                                                      shape=[paddle.shape(selected_fea_regions)[0], -1])
                ori_fea_regions[mask_regions_selected] = selected_fea_regions
                ori_fea_regions = paddle.reshape(ori_fea_regions, shape=[paddle.shape(x)[0], -1,
                                                                         self.num_cls * self.kernel_sizes[0] *
                                                                         self.kernel_sizes[1]])
                ori_fea_regions = paddle.transpose(ori_fea_regions, perm=[0, 2, 1])
                fea_out = F.fold(ori_fea_regions, [paddle.shape(x)[2], paddle.shape(x)[3]], self.kernel_sizes,
                                 strides=self.kernel_sizes[0], paddings=0, dilations=1, name=None)
