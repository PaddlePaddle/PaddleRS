...
class NL_Block(nn.Layer):
    ...
    def forward(self, x):
        ...
        sim_map = paddle.nn.functional.softmax(sim_map,
                                               axis=-1)  # B * (H*W) * (H*W)
...
class NL_FPN(nn.Layer):
    ...
    def __init__(self, in_dim, reduction=True):
        ...
        self.conv_e3 = nn.Sequential(
            nn.Conv2D(in_dim * 2, in_dim * 4, kernel_size=3,
                      stride=1, padding=1),
            nn.BatchNorm2D(in_dim * 4, momentum=bn_mom),
            nn.ReLU(),
        )
        ...
        self.conv_d2 = nn.Sequential(
            nn.Conv2D(in_dim * 2, in_dim, kernel_size=3,
                      stride=1, padding=1),
            nn.BatchNorm2D(in_dim, momentum=bn_mom),
            nn.ReLU(),
        )
        self.conv_d3 = nn.Sequential(
            nn.Conv2D(in_dim * 4, in_dim * 2, kernel_size=3,
                      stride=1, padding=1),
            nn.BatchNorm2D(in_dim * 2, momentum=bn_mom),
            nn.ReLU(),
        )
...
class cat(paddle.nn.Layer):
    def __init__(self, in_chn_high, in_chn_low, out_chn, upsample=False):
        ...
        self.conv2d = paddle.nn.Sequential(
            paddle.nn.Conv2D(in_chn_high + in_chn_low, out_chn,
...
class double_conv(paddle.nn.Layer):
    def __init__(self, in_chn, out_chn, stride=1,
        ...
        self.conv = paddle.nn.Sequential(
            paddle.nn.Conv2D(in_chn, out_chn, kernel_size=3,
                             stride=stride, dilation=dilation, padding=dilation),
            nn.BatchNorm2D(out_chn, momentum=bn_mom),
            paddle.nn.ReLU(),
            paddle.nn.Conv2D(out_chn, out_chn, kernel_size=3,
                             stride=1, padding=1),
            nn.BatchNorm2D(out_chn, momentum=bn_mom),
            paddle.nn.ReLU()
        )
...
class SEModule(nn.Layer):
    ...
    def __init__(self, channels, reduction_channels):
        ...
        self.fc1 = nn.Conv2D(
            channels, reduction_channels, kernel_size=1,
            padding=0, bias_attr=True)
        self.ReLU = nn.ReLU()
        self.fc2 = nn.Conv2D(
            reduction_channels, channels, kernel_size=1,
            padding=0, bias_attr=True)
...
class BasicBlock(nn.Layer):
    ...
    def __init__(self, inplanes, planes,
                 downsample=None, use_se=False, stride=1, dilation=1):
        ...
        self.conv2 = double_conv(first_planes, outplanes,
                                 stride=stride, dilation=dilation)
        self.se = SEModule(outplanes, planes // 4) if use_se else None
        self.downsample = paddle.nn.MaxPool2D(stride=2,
                                              kernel_size=2
...
class FCS(paddle.nn.Layer):
    def __init__(self, in_channels, os=16, use_se=False, **kwargs):
        ...
        # decoder
        self.decoder3 = cat(channel_list[0], channel_list[1], channel_list[1],
                            upsample=pool_list[0])
        self.decoder2 = cat(channel_list[1], channel_list[2], channel_list[2],
                            upsample=pool_list[1])
        self.decoder1 = cat(channel_list[2], channel_list[3], channel_list[3],
                            upsample=pool_list[2])

        self.df1 = cat(channel_list[3], channel_list[3], channel_list[3],
                       upsample=False)
        self.df2 = cat(channel_list[2], channel_list[2], channel_list[2],
                       upsample=False)
        self.df3 = cat(channel_list[1], channel_list[1], channel_list[1],
                       upsample=False)
        self.df4 = cat(channel_list[0], channel_list[0], channel_list[0],
                       upsample=False)
        ...
        self.conv_out = paddle.nn.Conv2D(8, 1,
                                         kernel_size=3, stride=1, padding=1)
...
class FCCDN(paddle.nn.Layer):
    ...
    def __init__(self,
        ...
        channel_list = [256, 128, 64, 32]
        # encoder
        self.block1 = BasicBlock(in_channels, channel_list[3], pool_list[3], se_list[3], stride_list[3], dilation_list[3])
        self.block2 = BasicBlock(channel_list[3], channel_list[2], pool_list[2], se_list[2], stride_list[2],
                                 dilation_list[2])
        self.block3 = BasicBlock(channel_list[2], channel_list[1], pool_list[1], se_list[1], stride_list[1],
                                 dilation_list[1])
        self.block4 = BasicBlock(channel_list[1], channel_list[0], pool_list[0], se_list[0], stride_list[0],
                                 dilation_list[0])
        ...
        self.conv_out = paddle.nn.Conv2D(8, 1, kernel_size=3, stride=1, padding=1)
        self.conv_out_class = paddle.nn.Conv2D(channel_list[3], 1, kernel_size=1, stride=1, padding=0)
