...
- `coco2mask.py`: Convert COCO annotated files to.png.
...
- `match.py`: Realize the registration of two images.
- `split.py`: Slice large format image data.
...
- `extract_ms_patches.py`: Extract multi-scale image blocks from the whole remote sensing image.
...
The main function of `coco2mask.py` is to convert images and corresponding COCO format split tags into images and .png format tags, and the results will be stored in `img` and `gt`  directories respectively. Relevant data samples can be found in [Case Data Set of Typical Urban Buildings in China].(https://www.scidb.cn/detail?dataSetId=806674532768153600&dataSetType=journal)。For mask, save the false-color image that results in a single channel. The usage is as follows:
...
- `ignore_index`：index values that need to be ignored in shapefile (such as background classes in split tasks) default to `255`.
