...
- `extract_ms_patches.py`: Extracts multi-scale image blocks from entire remote sensing images.
...
```shell
python mask2shape.py --srcimg_path {path to the original image with geographic information.} --mask_path {input segmentation label path} [--save_path {output vector image path}] [--ignore_index {index values to be ignored}]
```
...
- `im1_path`: The file path of the first temporal image. This image must contain geospatial information and will be used as the reference image during the registration process.
- `im2_path`: The file path of the second temporal image. The geospatial information of this image will not be used. This image will be registered to the first temporal image during the registration process.
- `im1_bands`: The bands of the first temporal image used for registration, specified as three channels (representing R, G, and B) or a single channel. Default is [1, 2, 3].
- `im2_bands`: The bands of the second temporal image used for registration, specified as three channels (representing R, G, and B) or a single channel. Default is [1, 2, 3].
- `save_path`: The output file path of the second temporal image after registration.
...
- `min_patch_size`: The minimum size of the extracted image block (in terms of the number of pixels in the length/width of the image block covered by the leaf nodes in the quadtree). Default is `256`.
- `bg_class`: The class ID of the background class. Default is `0`.
- `target_class`: The class ID of the target class. If it is `None`, it means that all classes except the background class are target classes. Default is `None`.
